...
`build_kali_registry()` provides fixed adapters for `nmap`, `dig`, `curl`, `ldapsearch`, `smbclient`, `ip`, `cat`, `nft`, and `timedatectl`. Commands run with `create_subprocess_exec` (no shell), a timeout, and the target policy. A CIDR entry such as `10.0.0.0/24` authorizes every address in that network. After an Nmap result, discovered IPv4 addresses are added to the runtime policy and reported as `[Target]`; this lets the ReAct agent continue with the discovered hosts. Install the corresponding Kali packages first, such as `nmap`, `dnsutils`, `curl`, `ldap-utils`, and `smbclient`.
