...
from cyberqa.common import message_to_json
...
class Logger:
    ...
    def __init__(
        ...
        if identity is not None and logger is not None:
            ...
            # Cochise keeps raw structured events, but each invocation gets an
            # isolated directory so reports and logs cannot be mixed between
            # cyber-range runs.
            self.run_directory = Path(run_directory).expanduser() if run_directory else None
            log_directory = (self.run_directory / "logs") if self.run_directory else Path("logs")
            log_directory.mkdir(parents=True, exist_ok=True)
            self.log_path = log_directory / f"run-{formatted_timestamp}.json"
