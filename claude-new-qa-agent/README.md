...
`build_kali_registry()` provides reviewed adapters for `nmap`, `nxc` (SMB/LDAP recon), Impacket, `dig`, `curl`, `ldapsearch`, `smbclient`, `ip`, `cat`, `nft`, and `timedatectl`. The default port probe is `nmap -sC -sV TARGET`; the LLM can select reviewed Nmap/NXC profiles or safe argv fragments (ports, timing, service/version flags, and read-only enumeration flags). The adapter validates those fragments and always injects the authorized target/module itself. Commands run with `create_subprocess_exec` (no shell), a timeout, and the target policy. A CIDR entry such as `10.0.0.0/24` authorizes every address in that network. After an Nmap result, discovered IPv4 addresses are added to the runtime policy and reported as `[Target]`; this lets the ReAct agent continue with the discovered hosts. The observation store is SQLite at `CYBERQA_OBSERVATION_DB` (default `.cyberqa/observations.sqlite3`) and records the effective argv, target, parameters, result, and failure across process restarts. Identical effective argv is cached and the initial baseline is bounded; deferred AD probes are selected only after evidence synthesis. `force_refresh=True` remains an explicit API/operator option and is not exposed to the model-visible tool schema. Install the corresponding Kali packages first, such as `nmap`, `netexec`, `impacket-scripts`, `dnsutils`, `curl`, `ldap-utils`, and `smbclient`.
