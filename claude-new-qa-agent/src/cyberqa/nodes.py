...
from .ad_knowledge_base import qa_checklist_prompt
from .ad_playbooks import capability_catalog, get_capability
from .ad_strategy import recommend as recommend_ad_method
from .memory_digest import consolidate_memory
from .situational import build_status_header
...
from .models import (ADRisk, Decision, Event, Evidence, Hypothesis, QAExpectations, Role,
                      Scorecard, Service, ServiceProtocol, ToolParameters, WeaknessVerdict)
...
SYSTEM = AD_QA_PLAYBOOK + """You are a senior Windows Active Directory QA/security engineer operating only
on authorized cyber-range targets. Reason like an experienced AD reviewer who knows which components,
functions, and configuration settings matter: DNS/naming context, LDAP signing and channel binding, SMB
signing and share ACLs, Kerberos pre-auth and encryption types, SPN/service-account hygiene, delegation
(unconstrained/constrained/RBCD), AD CS certificate templates and EKUs, trusts and SID filtering,
privileged-group and ACL edges, LAPS/GPO delegation, and password/lockout policy.

Use OODA: observe facts, orient against the objective and the consolidated memory_digest and prior
evidence, decide one justified action, and act through the supplied fact-only tools. ALWAYS orient on
memory_digest first: it states what is already known, proven, blocked, or unknown, which QA checks are
complete (do_not_repeat), and the recommended next checks. Never re-run a completed check or an identical
effective command; advance an unresolved QA check instead. Inspect every tool result before selecting the
next tool. Continue until QA coverage of the objective is complete.

Never invent facts, credentials, vulnerabilities, or successful attacks. For each component, verify the
relevant setting/prerequisite and state what is proven, blocked, or unknown. If no domain credentials
exist, do not repeat empty-credential SMB/LDAP/NXC probes; pivot to domain discovery, supplied
username-file AS-REP assessment, anonymous access only when evidence supports it, or another justified
path.
...
class Agents:
    ...
    def _project_observations(self, state: QAState, new_evidence: list[Evidence]) -> dict[str, Any]:
        ...
        memory_digest = consolidate_memory(
            knowledge=knowledge,
            target_profiles=profiles,
            evidence_synthesis=synthesis,
            recon_coverage=recon_coverage,
            method_history=state.get("method_history", []),
            evidence_sources=[item.source for item in all_evidence],
            has_credential=bool(knowledge.get("credentials_validated")),
        )
        return {"target_profiles": profiles, "evidence_synthesis": synthesis,
                "runtime_config": runtime, "ad_knowledge": knowledge,
                "recon_coverage": recon_coverage, "memory_digest": memory_digest}
    ...
    @staticmethod
    def _expectations(state: QAState) -> QAExpectations:
        """Resolve the configured QA baseline expectations for this run.

        Prefers an explicit ``qa_expectations`` in runtime_config (so a
        scenario file can supply it as data) and falls back to environment
        configuration. Never raises: an empty baseline is a valid result.
        """
        configured = (state.get("runtime_config") or {}).get("qa_expectations")
        if isinstance(configured, dict):
            try:
                return QAExpectations.model_validate(configured)
            except Exception:
                pass
        return QAExpectations.from_env()

    @staticmethod
    def _judge_scorecard_directive() -> str:
        return (
            "\n\nYou are the scenario judge. After reasoning over the evidence, your FINAL message "
            "must be a single JSON object (no prose, no code fences) with keys: "
            "\"solvable\" (bool), \"difficulty\" (one of too_easy|appropriate|too_hard|broken|unknown), "
            "\"scenario_status\" (short string), \"score\" (0-100 number), "
            "\"findings\" (array of short strings), and \"weaknesses\" (array of objects with "
            "id, expected, status one of present|absent|unverifiable|out_of_scope|inconclusive, "
            "reasoning, evidence_ids). Ground every verdict in the qa_expectations baseline and the "
            "collected evidence. Do NOT assume success: if the evidence does not confirm an expected "
            "item, mark it absent or unverifiable rather than present. If you cannot judge an item, "
            "use difficulty=unknown / status=inconclusive rather than guessing."
        )

    def _build_scorecard(self, state: QAState, proposal: dict[str, Any],
                         evidence: list[Evidence]) -> Scorecard:
        """Build a scorecard from the judge's grounded reasoning.

        Parses the judge LLM's final structured answer instead of stamping a
        constant. Falls back to an explicitly non-grounded 'unknown' scorecard
        when no structured verdict is available, so a missing/failed judge can
        never be mistaken for a clean pass.
        """
        raw = proposal.get("summary")
        parsed: dict[str, Any] = {}
        ...
        expectations = self._expectations(state)
        weaknesses: list[WeaknessVerdict] = []
        for item in parsed.get("weaknesses", []) or []:
            if not isinstance(item, dict):
                continue
            try:
                weaknesses.append(WeaknessVerdict.model_validate(item))
            except Exception:
                continue

        difficulty = parsed.get("difficulty")
        if difficulty not in {"too_easy", "appropriate", "too_hard", "broken", "unknown"}:
            difficulty = "unknown"
        score = parsed.get("score")
        try:
            score = max(0.0, min(100.0, float(score)))
        except (TypeError, ValueError):
            # No grounded score available. Do not invent a passing number.
            score = 0.0
        grounded = bool(parsed)
        expected_baseline_met: bool | None = None
        if weaknesses:
            expected_baseline_met = all(
                w.status in {"present", "out_of_scope"} for w in weaknesses
            )
        return Scorecard(
            solvable=bool(parsed.get("solvable", False)),
            difficulty=difficulty,
            scenario_status=str(parsed.get("scenario_status")
                                or ("evaluated" if grounded else "unevaluated")),
            findings=[str(f) for f in (parsed.get("findings") or [])],
            score=score,
            weaknesses=weaknesses,
            evidence_ids=evidence_ids,
            expected_baseline_met=expected_baseline_met,
            grounded=grounded,
        )
    ...
    async def _structured_supervisor(self, state: QAState) -> Decision:
        ...
        prompt = json.dumps({
            ...
            "evidence": [e.model_dump(mode="json") for e in state.get("evidence", [])[-8:]],
    ...
    def _react_graph(self, role: Role, state: QAState, instruction: str | None = None,
        ...
        if not has_ad_credentials and decision and decision.action not in {
        ...
        async def reason(s: dict[str, Any]) -> dict[str, Any]:
            ...
            self.progress("reasoning_start", agent=role.value)
            response = await model.ainvoke([
                SystemMessage(content=(SYSTEM + "\n\n" + build_status_header(state, role.value) + "\n"
                                       ...
                                       + (self._judge_scorecard_directive() if role == Role.JUDGE else ""))),
    ...
    def _planned_tool_for_action(decision: Decision | None) -> tuple[str, dict[str, Any]] | None:
        ...
        if "nmap" in text or "check_port" in text or "port_probe" in text:
            return "check_port", {
                key: params[key] for key in ("profile", "argv")
                if params.get(key)
            }
    ...
    async def specialist(self, role: Role, state: QAState) -> dict[str, Any]:
        ...
        if capability_tool:
            ...
            if action_tool and action_tool[0] in self.tools.tools:
                ...
                anonymous_tool = action_tool[0] in {"ldap_bind", "smb_negotiate",
                                                    "nxc_smb_recon", "nxc_ldap_recon"}
                if no_credentials and anonymous_tool and action not in {
                    "anonymous_identity_probe", "domain_inventory"
                }:
                    blocked_action = True
        ...
        elif self.llm and self.tools.tools:
                ...
                if isinstance(message, ToolMessage):
                    try:
                        ...
                        if payload.get("evidence"):
                            ...
                            if payload.get("needs_human") or observed.exit_code not in (None, 0):
                                inner_needs_human = True
                                proposal["needs_human"] = True
                                inner_human_request = {
                                    "kind": "tool_failure", "agent": role.value,
                                    "tool": payload.get("tool", message.name),
                                    "target": observed.target,
                                    "error": payload.get("error") or observed.stderr or "tool failed",
        ...
        elif self.tools.tools and not (action in {
            "analyze_existing_evidence", "summarize_evidence", "evaluate_ad_evidence"
        ...
        if role == Role.JUDGE:
            patch["scorecard"] = self._build_scorecard(state, proposal, evidence)
        return patch
